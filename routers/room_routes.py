...
async def create_room(room: RoomCreate, username: str = Depends(get_current_user)):
    return await storage.create_room(room, username)



@router.put("/update/", status_code=status.HTTP_200_OK)
async def update_room(room: RoomUpdate, username: str = Depends(get_current_user)):
...
async def user_request_join_room(room_id: str, username: str = Depends(get_current_user)):
