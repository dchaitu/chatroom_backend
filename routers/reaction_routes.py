...
async def create_reaction_to_message(reaction: ReactionDTO, username: str = Depends(get_current_user)):
    return await storage.create_reaction_to_message(reaction, username)


@router.get("/{room_id}/", response_model=List[UserReactionDTO])
async def get_reactions_to_messages_in_room(room_id: str):
