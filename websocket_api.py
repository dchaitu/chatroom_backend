...
def websocket_connect(event, context):
    ...
    if not username or not room_id:
        return {"statusCode": 400, "body": json.dumps({"message": "Missing username or room_id"})}
    ...
    user_in_room = db.query(User).filter(User.rooms.any(room_id=room_id)).filter(User.username == username).first()
    if not user_in_room:
        return {"statusCode": 403, "body": json.dumps({"message": "User not in room"})}

    existing_connection = db.query(Connection).filter(Connection.username == username, Connection.room_id == room_id).first()
    if existing_connection:
        db.delete(existing_connection)
        db.commit()

    new_connection = Connection(connection_id=connection_id, username=username, room_id=room_id)
